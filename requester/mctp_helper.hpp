...
class MctpIoClass : public IOClass
{
  public:
     ...
     *
     * @return true if the socket file descriptor is valid, false otherwise.
     */
  ...
  private:
     ...
     */
    libspdm_return_t writeToSocket(const std::vector<uint8_t>& buf)
    {
        if (!isSocketOpen())
        {
     ...
     */
    libspdm_return_t readFromSocket(std::vector<uint8_t>& buf)
    {
        if (!isSocketOpen())
        {
        ...
        std::array<uint8_t, 3> head{1, addr.smctp_addr.s_addr, MCTP_TYPE_SPDM};
