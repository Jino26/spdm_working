...
class SpdmMctpTransport : public SpdmTransport
{
  ...
  private:
    ...
    /**
     * @brief Shared callback to acquire the send/receive buffer.
     *        Used for both sender and receiver buffer registration.
     *
     * @param context The SPDM context
     * @param msg_buf_ptr Pointer to store the allocated buffer
     * @return libspdm_return_t Status code indicating success or failure
     */
    static libspdm_return_t acquireBuffer(void* context, void** msg_buf_ptr);

    /**
     * @brief Shared callback to release the send/receive buffer.
     *        Used for both sender and receiver buffer registration.
     *
     * @param context The SPDM context
     * @param msg_buf_ptr Pointer to the buffer being released
     */
    static void releaseBuffer(void* context, const void* msg_buf_ptr);
};
