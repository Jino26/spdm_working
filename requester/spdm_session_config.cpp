                                               ...
                                               const SecureSessionConfig& cfg)
{
    ...
    if (!cfg.peerRootCertBaseDir.empty())
    {
        ...
        const fs::path certPath =
            fs::path(cfg.peerRootCertBaseDir) / subdir / cfg.peerRootCertFileName;
        return loadCertFromFile(certPath, std::format("algo subdir: {}", subdir));
    }
                                     ...
                                     const SecureSessionConfig& cfg)
{
    ...
    if (!certResult)
    {
        ...
        const bool configAttempted = !cfg.peerRootCertDer.empty() ||
                                     !cfg.peerRootCertDerPath.empty() ||
                                     !cfg.peerRootCertBaseDir.empty();

        if (configAttempted)
        {
            // Configuration was provided but loading failed (error already logged)
            return LIBSPDM_STATUS_INVALID_PARAMETER;
        }
    ...
    libspdm_data_parameter_t p{};
    p.location = LIBSPDM_DATA_LOCATION_LOCAL;
    const libspdm_return_t st = libspdm_set_data(
        ctx, LIBSPDM_DATA_PEER_PUBLIC_ROOT_CERT, &p, certResult->data.data(),
        certResult->data.size());
