...
libspdm_return_t SpdmSession::heartbeat()
{
    ...
    return libspdm_heartbeat(transport.spdmContext.get(), id);
}
...
libspdm_return_t SpdmSession::keyUpdate(bool singleDirection)
{
    ...
    return libspdm_key_update(transport.spdmContext.get(), id, singleDirection);
}
                                   ...
                                   std::span<uint8_t> resp, size_t& respLen)
{
    ...
    respLen = resp.size();
    return libspdm_send_receive_data(transport.spdmContext.get(), &id,
